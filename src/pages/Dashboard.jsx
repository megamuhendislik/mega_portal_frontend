        ...
        <div className="space-y-8">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
                <div className="flex items-center justify-between mb-6">
                    ...
                    {todaySummary && todaySummary.is_working && (
                ...
                {todaySummary ? (
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        ...
                        <div className="bg-slate-50 rounded-lg p-4 border border-slate-100">
                            ...
                            <div className="flex items-end justify-between">
                                <h4 className="text-2xl font-bold text-slate-800">
                                    {Math.floor(todaySummary.remaining_work / 60)}s {todaySummary.remaining_work % 60}dk
                                </h4>
                                <span className="text-xs text-slate-400 mb-1">
                                    / {Math.floor(todaySummary.daily_expected / 60)}s {todaySummary.daily_expected % 60}dk
                                </span>
                            </div>
                            <div className="w-full bg-slate-200 rounded-full h-2 mt-3">
                                <div
                                    className="bg-blue-600 h-2 rounded-full transition-all duration-500"
                                    style={{ width: `${Math.min(100, (todaySummary.total_worked / todaySummary.daily_expected) * 100)}%` }}
                                ></div>
                            </div>
                        </div>
                        ...
                        <div className="bg-slate-50 rounded-lg p-4 border border-slate-100">
                            ...
                            <div className="flex items-end justify-between">
                                <h4 className="text-2xl font-bold text-slate-800">
                                    {todaySummary.remaining_break} dk
                                </h4>
                                <span className="text-xs text-slate-400 mb-1">
                                    Kullanılan: {todaySummary.break_used} dk
                                </span>
                            </div>
                            <div className="w-full bg-slate-200 rounded-full h-2 mt-3">
                                <div
                                    className={`h-2 rounded-full transition-all duration-500 ${todaySummary.remaining_break < 0 ? 'bg-red-500' : 'bg-emerald-500'}`}
                                    style={{ width: `${Math.min(100, (todaySummary.break_used / (todaySummary.break_used + todaySummary.remaining_break || 30)) * 100)}%` }}
                                ></div>
                            </div>
                        </div>
                        ...
                        <div className="bg-slate-50 rounded-lg p-4 border border-slate-100">
                            ...
                            <div className="flex items-end justify-between">
                                <h4 className={`text-2xl font-bold ${todaySummary.current_overtime > 0 ? 'text-amber-600' : 'text-slate-800'}`}>
                                    {todaySummary.current_overtime} dk
                                </h4>
                                {todaySummary.current_overtime > 0 && (
                                    <TrendingUp size={16} className="text-amber-500 mb-1" />
                                )}
                            </div>
                            <p className="text-xs text-slate-400 mt-3">
                                {todaySummary.current_overtime > 0 ? 'Normal mesai tamamlandı.' : 'Henüz ek mesai yok.'}
                            </p>
                        </div>
                    </div>
                ) : (
