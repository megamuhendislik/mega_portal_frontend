...
const SeverityBadge = ({ severity }) => {
    const map = {
        HIGH: { bg: 'bg-red-100 text-red-800 border-red-200', label: 'Yüksek' },
        MEDIUM: { bg: 'bg-amber-100 text-amber-800 border-amber-200', label: 'Orta' },
        LOW: { bg: 'bg-blue-100 text-blue-800 border-blue-200', label: 'Düşük' },
    };
...
const SourcePill = ({ source }) => {
    ...
    return (
        <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold border ${map[source] || 'bg-gray-50 text-gray-600 border-gray-200'}`}>
            {source === 'INTENDED' ? 'Planlı' : source === 'MANUAL' ? 'Manuel' : 'Algılanan'}
        </span>
    );
};
...
export default function OvertimeAuditTab() {
    ...
    const handlePurge = async (purgeType, pattern = null) => {
        if (!window.confirm(`Bu işlemi geri alamazsınız. ${purgeType} verilerini temizlemek istediğinize emin misiniz?`)) return;
        ...
        try {
            ...
            alert(`${res.data.deleted_count} kayıt temizlendi.`);
            runAudit(); // refresh
        } catch (err) {
    ...
    // Column definitions
    const reqColumns = [
        { header: 'Çalışan', key: 'employee_name', render: r => <span className="font-bold text-gray-800">{r.employee_name}</span> },
        ...
        { header: 'Oluşturulma', key: 'created_at', render: r => <span className="font-mono text-[10px]">{r.created_at || '—'}</span> },
        { header: 'Flags', render: r => r.flags?.length > 0 ? <div className="flex flex-wrap gap-1">{r.flags.map((f,i) => <FlagBadge key={i} flag={f} />)}</div> : '—' },
    ];

    const assignColumns = [
        { header: 'Çalışan', render: r => <span className="font-bold text-gray-800">{r.employee_name}</span> },
        { header: 'Departman', key: 'department' },
        ...
        { header: 'Oluşturulma', render: r => <span className="font-mono text-[10px]">{r.created_at || '—'}</span> },
        { header: 'Flags', render: r => r.flags?.length > 0 ? <div className="flex flex-wrap gap-1">{r.flags.map((f,i) => <FlagBadge key={i} flag={f} />)}</div> : '—' },
    ];
        ...
        <div className="space-y-6">
                ...
                <>
                    ...
                    >
                        {data.anomalies?.length === 0 ? (
                            <div className="flex items-center gap-2 text-sm text-green-700 py-3">
                                <CheckCircleIcon className="w-5 h-5" /> Anomali bulunamadı — veriler temiz.
                            </div>
                        ) : (
                                ...
                                {data.anomalies.map((a, i) => (
                                    <div key={i} className="flex items-center justify-between bg-gray-50 rounded-lg p-3 border border-gray-100">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <span className="text-xs text-gray-400">({a.count} kayıt)</span>
                                        </div>
                                        <div className="flex items-center gap-2">
                                            ...
                                            {a.type === 'FUTURE_ATTENDANCE_OT' && (
                                                ...
                                                >
                                                    <TrashIcon className="w-3.5 h-3.5" /> Sıfırla
                                                </button>
                                            )}
                    ...
                    >
                        ...
                        {filteredRequests.length > 100 && (
                            <p className="text-[10px] text-gray-400 mt-2 text-center">İlk 100 kayıt gösteriliyor (toplam: {filteredRequests.length})</p>
                        )}
                    </Section>
                    ...
                    >
                        ...
                        />
                        <DataTable columns={attColumns} data={filteredAttendance} emptyText="Filtreye uyan mesai kaydı yok" />
                    </Section>
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                ...
                                {data.summary.suspicious_patterns && Object.keys(data.summary.suspicious_patterns).length > 0 && (
                                    <div className="col-span-2">
                                        <h4 className="text-[10px] font-bold text-red-500 uppercase mb-2">Şüpheli Pattern'ler</h4>
                                        <div className="space-y-1">
            ...
            {!data && !loading && (
                <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-12 text-center">
                    ...
                    <p className="text-xs text-gray-400 mt-1">"Denetimi Başlat" butonuna tıklayarak tüm mesai verilerini tarayabilirsiniz.</p>
                </div>
            )}
        </div>
    );
