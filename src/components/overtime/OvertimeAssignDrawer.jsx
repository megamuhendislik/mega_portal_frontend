...
import {
  X, Clock, AlertTriangle, CalendarPlus, Loader2,
...
export default function OvertimeAssignDrawer({ open, onClose, employee, onSuccess }) {
  // Data state
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const [fiscalRes, busyRes, weeklyRes] = await Promise.allSettled([
        ...
        api.get(`/overtime-assignments/busy-days/${employee.id}/`),
  ...
  // --- Total calculation text (only show when more than 1 day selected) ---
  ...
  return (
    ...
    >
      {/* ===== Success overlay (rendered inside main Drawer to avoid unmount/remount flicker) ===== */}
      {success ? (
        ...
        {!loading && (
          <div className="px-5 py-3 border-t border-slate-100 flex items-center gap-3">
            <button
              ...
              onClick={onClose}
              disabled={submitting}
              className="flex-1 py-2.5 font-bold text-slate-500 hover:bg-slate-50 rounded-xl text-sm transition-colors disabled:opacity-50"
            >
              İptal
            </button>
            <button
              ...
              className={`flex-1 py-2.5 font-bold rounded-xl text-sm shadow-lg transition-all disabled:opacity-50 flex items-center justify-center gap-2 ${getSubmitButtonClasses()}`}
            >
              {submitting ? (
                <>
