...
/* ----- Status Badge for OvertimeRequests ----- */
...
export default function MyOvertimeSubTab() {
    ...
    const fetchData = async () => {
        try {
            ...
            console.error('MyOvertimeSubTab fetch error:', err);
            setError('Mesai verileri yuklenemedi.');
            setAssignments([]);
            setClaimableData({ intended: [], potential: [] });
            setMyOvertimeRequests([]);
    ...
    const submitCancel = async () => {
        try {
            await api.post(`/overtime-requests/${cancelTarget.id}/cancel/`, {
                reason: cancelReason || undefined,
            });
            setCancelTarget(null);
            setCancelReason('');
            setSuccessMsg('Talep basariyla iptal edildi.');
            setTimeout(() => setSuccessMsg(''), 4000);
            fetchData();
        } catch (err) {
            alert(err.response?.data?.error || 'Iptal sirasinda hata olustu.');
        }
    };
                                            ...
                                            <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
                                                ...
                                                {item.status === 'PENDING' && (
                                                    <button
                                                        ...
                                                        className="px-3 py-1.5 bg-red-50 hover:bg-red-100 text-red-600 font-bold text-xs rounded-lg transition-colors flex items-center gap-1 shrink-0"
                                                    >
                                                        <XCircle size={14} />
                                                        Iptal Et
                                                    </button>
            ...
            {cancelTarget && ReactDOM.createPortal(
                <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4 animate-in fade-in">
                    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 space-y-4">
                        <h3 className="text-lg font-bold text-slate-900 flex items-center gap-2"><XCircle size={20} className="text-red-500" /> Talebi Iptal Et</h3>
                        <p className="text-sm text-slate-600">
                            <strong>{formatDateTurkish(cancelTarget.date)}</strong> tarihli ek mesai talebinizi iptal etmek istediginize emin misiniz?
                        </p>
                        <textarea
                            rows="2"
                            value={cancelReason}
                            onChange={e => setCancelReason(e.target.value)}
                            placeholder="Iptal sebebi (opsiyonel)"
                            className="w-full p-3 bg-slate-50 rounded-xl border border-slate-200 text-sm resize-none focus:ring-2 focus:ring-red-200 focus:border-red-300 outline-none"
                        />
                        <div className="flex gap-2">
                            <button onClick={() => setCancelTarget(null)} className="flex-1 py-2.5 font-bold text-slate-500 hover:bg-slate-50 rounded-xl text-sm">Vazgec</button>
                            <button onClick={submitCancel} className="flex-1 py-2.5 bg-red-600 hover:bg-red-700 text-white font-bold rounded-xl text-sm shadow-lg shadow-red-500/20">Iptal Et</button>
                        </div>
                    </div>
                </div>,
                document.body
